...
int run_game(int argc, char** argv, const RunOptions& opts) {
    ...
    gba::g_ws_authored_margin_layers = opts.ws_authored_margin_layers;
    ...
    auto take_autosave = [&]() {
        ...
        std::string e;
        if (do_savestate_save(path, e)) {
            std::printf("autosave slot=%d path=\"%s\"\n",
                        autosave_next_slot + 1, path.c_str());
            std::fflush(stdout);
    ...
    for (int i = 0; i < step_budget && !host_quit; ++i) {
        ...
        if (autosave_interval_frames) {
            ...
            if (fc_now - autosave_last_frame >= autosave_interval_frames)
                take_autosave();
        }
